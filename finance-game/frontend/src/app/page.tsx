...
export default function FinanceGame() {
  ...
  const calculateDefaultExpenses = (monthlySalary: number) => {
    return {
      state: '',
      healthInsurance: Math.max(monthlySalary * 0.15, 0), // 15% of monthly salary
      utilities: Math.max(monthlySalary * 0.10, 0), // 10% of monthly salary
      groceries: Math.max(monthlySalary * 0.20, 0), // 20% of monthly salary
      rent: Math.max(monthlySalary * 0.30, 0), // 30% of monthly salary
      internetCable: Math.max(monthlySalary * 0.05, 0), // 5% of monthly salary
      gymFitness: Math.max(monthlySalary * 0.02, 0), // 2% of monthly salary
      streamingServices: Math.max(monthlySalary * 0.02, 0), // 2% of monthly salary
      customSubscription: {
        name: '',
        amount: 0
      },
      loans: {
        carLoan: Math.max(monthlySalary * 0.15, 0), // 15% of monthly salary
        studentLoan: Math.max(monthlySalary * 0.15, 0), // 15% of monthly salary
        homeLoan: Math.max(monthlySalary * 0.25, 0) // 25% of monthly salary
      }
    };
  };
  ...
  const handleChoice = (choice: Option) => {
    ...
    if (currentMonthRound >= 4) { // Always 4 rounds per month
  ...
  const getMonthlyData = () => {
    const monthlyData = months.map(month => {
      const monthEntries = gameHistory.filter(entry => entry.month === month.name);
    ...
    return (
      <div className="mt-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-semibold text-black">Financial Progress</h3>
          <div className="flex items-center space-x-2">
            <button
              onClick={() => setViewMode('monthly')}
              className={`px-3 py-1 rounded-md ${viewMode === 'monthly'
            ...
            <button
              ...
              className={`px-3 py-1 rounded-md ${viewMode === 'round'
                ? 'bg-blue-600 text-white'
                : 'bg-gray-200 text-gray-700'
                }`}
            >
        ...
        {/* History Section */}
        <h3 className="text-xl font-semibold mb-4 text-black">Detailed History</h3>
            ...
            monthlyData.map((entry, index) => (
              <li key={index} className="bg-gray-50 p-3 rounded-md text-black">
                <div className="border-b pb-2 mb-2">
                  ...
                  <p className={`font-medium ${entry?.capitalChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  ...
                  <div>
                    ...
                    {entry?.decisions.map((decision, idx) => (
                      <div key={idx} className="ml-4 mb-2">
                        <p className="text-sm">Scenario: {decision.description}</p>
  ...
  const renderExpensesForm = () => (
    <form onSubmit={handleExpensesSubmit} className="space-y-6 max-w-xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Monthly Income & Expenses</h2>
        <button
          type="button"
      ...
      {/* Month Selection */}
      <div className="mb-6">
        <label className="block text-black font-bold text-lg mb-2">Number of Months:</label>
        <select
      ...
      <div className="mb-6">
        ...
        <select
          ...
          onChange={(e) => setMonthlyExpenses({ ...monthlyExpenses, state: e.target.value })}
      ...
      <div className="bg-gray-50 p-4 rounded-lg space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          {Object.entries({
            ...
            <div key={label}>
              <label className="block text-black mb-2">{label} {showPercentages ? '(%)' : '($)'}:</label>
              <input
                type="number"
                value={showPercentages ? getPercentage(value, monthlySalary) : value}
                onChange={(e) => {
                  const newValue = parseFloat(e.target.value) || 0;
                  const actualValue = showPercentages
        ...
        <div className="grid grid-cols-2 gap-4">
          {Object.entries({
            ...
            <div key={label}>
              ...
              <input
                type="number"
                value={showPercentages ? getPercentage(value || 0, monthlySalary) : value}
                onChange={(e) => {
                  const newValue = parseFloat(e.target.value) || 0;
                  const actualValue = showPercentages
                    ? (monthlySalary * (newValue / 100))
      ...
      <div className="bg-yellow-50 p-4 rounded-lg mt-4">
        <p className="text-sm text-black">
          <strong>Note:</strong> Your monthly salary will be added and expenses will be deducted at the end of each month.
          Salary will increase by 5% each month to account for career growth. All default expenses are calculated as a percentage of your monthly salary.
        </p>
      </div>
  ...
  const generateScenarios = async () => {
    ...
    try {
      ...
      const fixedCostMax = startingCapital * 0.10;
...
Generate a JSON array with EXACTLY ${totalScenarios} scenario objects (${selectedMonths} months × 4 scenarios per month).
...
Output only a valid JSON array without any extra text.`;
      ...
      if (generatedScenarios.length !== totalScenarios) {
        console.error(`Expected ${totalScenarios} scenarios but got ${generatedScenarios.length}`);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8 flex">
      <div className={`transition-all duration-300 ${showAnalysis ? 'w-2/3 pr-4' : 'w-full'}`}>
        <div className="bg-white rounded-lg shadow-md p-6 h-[calc(100vh-4rem)] overflow-y-auto">
          {gameOver ? (
            <div>
              <h2 className={`text-2xl font-bold mb-4 ${win ? "text-green-600" : "text-red-600"}`}>
            ...
            showExpensesForm ? renderExpensesForm() : (
            ...
            <div className="space-y-4">
              <h2 className="text-xl font-bold text-center mb-4 text-gray-900">Round Result</h2>
